...
class SearchInRotatedSortedArray2 {
    ...
    public static void main(String[] args) {
        SearchInRotatedSortedArray2 searcher = new SearchInRotatedSortedArray2();
        int[] nums = {2, 5, 6, 0, 0, 1, 2};
        int target = 0;
        System.out.println(searcher.search(nums, target)); // true

        target = 3;
        System.out.println(searcher.search(nums, target)); // false

        int[] nums2 = {1, 0, 1, 1, 1};
        target = 0;
        System.out.println(searcher.search(nums2, target)); // true
    }
